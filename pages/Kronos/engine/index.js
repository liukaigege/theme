...
engine.registerFilter("offShow", ($defaultPrice, $originalPrice) => {
	return parseFloat(
		Math.floor(
			(1 - Number($defaultPrice) / Number($originalPrice)) * 100
		) || 1
	);
});
engine.registerFilter("relationList", ($res, $categoryData, $data) => {
	...
	function category($res, $res_id, relationListArr) {
		// 关联商品不够12个,在分类中取
		if (
			$res.hasOwnProperty("categoryIds") &&
			$res.categoryIds != "" &&
			$res.categoryIds != null
		) {
			...
			$categoryIdsArr.forEach((element) => {
				...
				if (categoryArr.length) {
					...
					if (categoryNewArr.length > relationListArrLength) {
						...
						relationListArr =
							relationListArr.concat(categoryNewArr);
					}
				}
			});
		}
		// 如果分类不够去标签取
		if (
			relationListArr.length < 12 &&
			$res.hasOwnProperty("tag") &&
			$res.tag != "" &&
			$res.tag != null
		) {
			let $tag = $res.tag;
		...
		if (
			relationListArr.length < 12 &&
			$res.hasOwnProperty("productClassify") &&
			$res.productClassify != "" &&
			$res.productClassify != null
		) {
			let $productClassify = $res.productClassify;
			let productClassifyData = $data.filter((item) => {
				let productClassify = "";
				if (
					item.hasOwnProperty("productClassify") &&
					item.productClassify != "" &&
					item.productClassify != null
				) {
			...
			let productClassifyData_copy = JSON.parse(
				JSON.stringify(productClassifyData)
			);
			// 可以加入数组的最大数量
			let relationListArrLength = 12 - relationListArr.length;
...
engine.registerFilter("pageList", (v) => {
	...
	if (isFind) {
		...
		result =
			v.bottom.linkUrl.toLocaleLowerCase().indexOf("terms") != -1
				? v.bottom.linkUrl
				: "";
	}
	return result;
