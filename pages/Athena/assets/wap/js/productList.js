...
if (isAll == 0) {
  ...
  searchResult = categoryData ? categoryData.goodsList.filter((item) => exemptSearchList.indexOf(item.id) === -1) : []// 获取当前分类下的数据 // 去改分类下的商品数据（改商品数据包含了子分类的数据）
...
function changePositionToNotJuxta() {
  position = false
  $('.athena_product_item').css({ width: '100%' })
  ...
  $('.athena-productListBox img').css({ width: 'auto', height: 'auto' })
}
function changePositionToJuxta() {
  position = true
  $('.athena_product_item').css({ width: '48%' })
  $('.athena_product_item .athena_product_introduce_title ').css({
    'font-size': '0.1rem',
  })
  $('.athena_product_item .defaultPrice ').css({
