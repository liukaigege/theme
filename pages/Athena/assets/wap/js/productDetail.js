  ...
  .use(['jquery', 'layer', 'laytpl', 'form'], function () {
    ...
    */
    ...
    function comments() {
      ...
      $('.comments_btn').on('click', function (event) {
        event.stopPropagation()
    ...
    */
    function productDetails() {
      /* 商品信息详情描述 */
      let description = $('.description').attr('data-description')
      if (
        description &&
        description != '' &&
        Object.prototype.toString.call(description).slice(8, -1) != 'Null'
      ) {
        $('.description .d_text').load(
          '/static-page' + description + '?version=' + new Date().getTime(),
          '',
          function () { }
        )
      }
        ...
        if (res[i].id == productId) {
          ...
          function baseCoupon() {
            /*
                        循环所有优惠过期直接删除，其他的展示
                    */
            $('.pro_details .coupon li').each(function (index, el) {
            ...
            /*
                        点击领取优惠券
                    */
            $('.pro_details .coupon').on('click', 'li', debounce(function (event) {
              event.stopPropagation()
