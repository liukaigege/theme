...
/* 
  设置网站标题
  首先获取具体页面名称 index.html 和根路径是同一个页面，
  然后拼接设置title 其他页面逻辑一样
*/
...
/*  
  图片转换前提: 浏览内核Safari返回.jpg 其他返回.webp
  1、若图片url含有cdn.shopifycdn.net不作转换
  2、若图片url含有vshop、.jpg、.webp、.JPG进行转换
  3、其他不作转换
*/
...
function analyzeData(eventVal, edata, goodsList) {
  ...
  if (getStorage('shopid') && Object.prototype.toString.call(getStorage('shopid')).slice(8, -1) != 'Null' &&
    getStorage('oldDynamicId') && Object.prototype.toString.call(getStorage('oldDynamicId')).slice(8, -1) != 'Null') {
    sendObj.shop = getStorage('shopid');
...
/*
  上一跳具体值 facebook、google、instagram、direct
*/
...
//比较二个时间节点(时间值为字符串的年月日)
function contrastTime(isChange, endTime, startTime) {
  let now = new Date().getTime()
  if (isChange) {
    let sTime = new Date(startTime.replace(/T/g, ' ').replace(/-/g, '/')).getTime()
    let eTime = new Date(endTime.replace(/T/g, ' ').replace(/-/g, '/')).getTime()
    if (
...
function contrastTimeZone(isChange, endTime, startTime) {
  ...
  if (isChange) {
    let sTime =
      new Date(startTime.replace(/T/g, ' ').replace(/-/g, '/')).getTime() +
      new Date().getTimezoneOffset() * 60 * 1000 // 获取格林威治时间戳 正常时间戳（yyyy-mm-dd）会加上时区时间戳
    let eTime =
      new Date(endTime.replace(/T/g, ' ').replace(/-/g, '/')).getTime() +
...
if (!getStorage('currentRate')) {
  topSetRate($('.m_header_currency_list li[m-maincurrency="1"]'))
} else {
  let currentRate = JSON.parse(getStorage('currentRate'))
  $('.m_header_currency_list li').each(function (index, item) {
    if ($(item).attr('m-currency') == currentRate.currency) {
      topSetRate($(item))
    }
  })
}
...
/*
  货币存localStorage逻辑
*/
function topSetRate($dom) {
  let $currency = $dom.attr('m-currency'),
    $symbol = $dom.attr('m-symbol'),
    $rate = $dom.attr('m-rate'),
    $pic = $dom.attr('m-pic')
  let $m_json = {
    currency: $currency,
    symbol: $symbol,
  ...
  setStorage('currentRate', JSON.stringify($m_json))
}
...
function descr(descr) {
  ...
  if (descr.indexOf('**') != -1) {
    ...
    let mSymbol = JSON.parse(getStorage('currentRate')).symbol || ''
    sDescr =
      preDecr + mSymbol + formatMoney(getSpaceMoney(upMoeny).split(' ')[1])
  } else {
    sDescr = descr
  }
  return sDescr
  ...
  $(function () {
    ...
    $dom.each(function (index, el) {
      ...
      if (
        suffixImg &&
        suffixImg != '' &&
        Object.prototype.toString.call(suffixImg).slice(8, -1) != 'suffixImg'
      ) {
...
if (ua.match(/iPhone\sOS/i) !== 'iphone os') {
  window.onresize = function () {
    if (
      ...
      document.querySelector('#keyword') != document.activeElement
    ) {
      setTimeout(function () {
        // var top = document.activeElement.getBoundingClientRect().top / 2;
        // window.scrollTo(0, top);
        document.activeElement.id &&
          document
            .querySelector('#' + document.activeElement.id)
...
$(function () {
  ...
  /* 
  productDetail.html(商品详情页)需要本页面拼接document.title以后在发送
  其他页面直接发送
*/
  if (
    ($pathname[$pathname.length - 1] &&
