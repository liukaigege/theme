...
function shippingCost(res) {
  ...
  if ($res) {
    ...
    if ($res.gapMoney) {
      let gapMoney = `Livraison gratuite pour 10 livres de plus <i class="total_price">${getMoney(res.gapMoney)}</i> livres de plus.`
      $('.cart_head span').html(gapMoney)
      ...
      success(res) {
        if (res.code === 0) {
          ...
          if (!cartInfo.cartList.length) {
            let gapMoney = `<i>0/i>0 articles dans votre panier`
            $('.cart_head span').html(gapMoney)
            $('#cart').hide()
            $('.cartEmpty').show()
          }
          ...
          if ($item.attr('data-wholesale') == 1) {
            ...
            cart_info.goodsList.forEach(item => {
              ...
              $item.find('.off i').text(parseFloat(Math.floor(100 - Number(item.discountPrice) / Number(item.sellingPrice) * 100)))
            })
          }
          // 判断全场活动
          const promotionList = CONFIGDATA.activities.find(item => {
  ...
  getToken(function () {
    paypal.Buttons({
        ...
        return fetch('/vshop-site/api/v1/pre/orders/orders', {
          ...
          if (orderData.code != 0) {
            if (orderData.msg === 'The total amount is zero') {
              let tip = "Votre demande n'a pas pu être exécutée parce que la validation de l'entreprise était incorrecte et a échoué. Le montant ne doit pas être inférieur à zéro.";
              alertTxt(tip)
            } else {
      ...
      onApprove: function (data, actions) {
        $('html').append('<div class="paymentLoading"><div><img src="../assets/wap/images/icon/loading.gif" alt=""><p>Veuillez patienter un moment avant le paiement......</p><div></div>')
...
$(document).on('click', '.cart_icon--close,[data-cart="close_cart"]', function () {
  ...
  $('.header-warp').removeClass('cart-fixed')
})
...
if (navigator.userAgent.toLowerCase().match(/iPhone\sOS/i) !== "iphone os") {
  ...
  window.addEventListener('resize', () => {
    ...
    if (innerHeight > newInnerHeight) {
      // 键盘弹出事件处理
      $('.cart_checkout').css('position', 'static')
