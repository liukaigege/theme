          ...
          if (res.code == 0) {
              ...
              <li class="radio-wrapper payssionLists" data-method-name="payssion">
                <label class="radio__label" for="payssionMethod">
                  <div class="radio__input">
                    <input class="input-radio  name="choosepayment" payssionMethod" type="radio" value="false">
                  </div>
                  <span>${v.methodName} via payssion</span>
                </label>
                <div class="paymentMethod_btn">
                  <div>Cliquez sur le bouton ci - dessous pour payer et vous serez redirigé vers la page de paiement pour compléter votre achat en toute sécurité. Contactez - nous si vous avez besoin d'aide.</div>
                  <div class="payssionPay showPayssionPay" data-id="${v.id}">Continuez à vérifier.</div>
                </div>
              </li>
              `
              });
            ...
            <li class="radio-wrapper payssionLists" data-method-name="payssion">
              ...
              <div class="paymentMethod_btn">
                ...
                <div>Cliquez sur le bouton ci - dessous pour payer et vous serez redirigé vers la page de paiement pour compléter votre achat en toute sécurité. Contactez - nous si vous avez besoin d'aide.</div>
                <div class="payssionPay listPayssionPay showPayssionPay" data-id="${lastArr[0].id}">Continuez à vérifier.</div>
              </div>
            </li>
            `
...
function codPayShowFn(countryCode) {
  ...
  if (codStatus) {
    ...
    $('#codList').hide();
    if ($('#codList').find('#codMethod').prop('checked')) {
      $('#codList').find('#codMethod').prop('checked', false);
      $('#codList').find('.codPay').hide();
      $('.paypalBtns').show();
      $('#paypalMethod').prop('checked', true);
    }
  }
}
$('#codList .codPay').on('click', function (e) {
  e.stopPropagation();
    ...
    $.req({
      ...
      success(res) {
        if (res.code === 0) {
          ...
          alertTxt('Anomalies de paiement')
...
function product(res) {
  layui.use(['laytpl'], function () {
    ...
    let getOrderTpl = order_info_tempalte.innerHTML,
      orderView = document.getElementById('order-info')
...
$('.billing-address-ul').on('click', 'li', function (e) {
  ...
  if (typeName === 'billing-address') {
    ...
    $('#billing_address_form select').removeClass('is-change')
...
$(document).on('click', '.adress_btn', function () {
  ...
  if (issubmit) {
    ...
    $("html,body").animate({
      scrollTop: $("#payment li.err_active:first").offset().top - 200
    }, 500)
    ...
    for (var key in address) {
      if (key === 'country') {
        ...
        if (address['countryCode'] === 'CAN' || address['countryCode'] === 'USA') {
          $('#' + $id + ' [data-stata="' + address['countryCode'] + '"').find('select').addClass('is-change')
...
$('#paymentMethod').on('click', '.radio-wrapper', function (e) {
  ...
  if (paymentName === 'paypal') {
    ORDER_PAYMENT.payMethod = 'paypal'
    if (ORDER_PAYMENT.hasOwnProperty('paymentMethodInfo')) {
    ...
    onApprove: function (data, actions) {
      ...
      return fetch('/vshop-third/api/v1/third/' + data.orderID + '/capture', {
        ...
        if (errorDetail) {
          ...
          return alertTxt('Anomalies de paiement')
        }
  ...
  getToken(function () {
    ...
    var handleAction = function (clientSecret) {
      stripe.handleCardAction(clientSecret).then(function (data) {
        ...
        if (data.error) {
          showError("Votre carte n'est pas authentifiée, Veuillez réessayer");
...
if (alipayStatus) {
  ...
  $('#payment_button:not([disabled])').on('click', function () {
    if (isClickAlipay) {
      isClickAlipay = false
