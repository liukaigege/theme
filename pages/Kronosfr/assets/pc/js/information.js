...
clearTimeout(timer);
$(document).on('click', '#order_checkout:not([disabled])', function () {
  if (createOrderStatus) {
    ...
    //判断是否选中省份
    if ($('#shipping_address_form .province-cell.active').hasClass('province-input')) {
      if ($('#shipping_address_form .province-cell.active').find('.field__input').val().trim() === '') {
      ...
      if ($('#shipping_address_form .province-cell.active').find('.field__input').val() == null) {
        $('#shipping_address_form .province-cell.active').closest('li').addClass('err_active');
        msgErrAnimate($('#shipping_address_form .province-cell.active').find('.msg'));
        issubmit = false;
      }
    ...
    if ($('.billing-address-ul [data-type="billing-address"]').hasClass('active')) {
      ...
      //判断是否选中省份(账单地址)
      if ($('#billing_address_form .province-cell.active').hasClass('province-input')) {
        if ($('#billing_address_form .province-cell.active').find('.field__input').val().trim() === '') {
        ...
        if ($('#billing_address_form .province-cell.active').find('.field__input').val() == null) {
          $('#billing_address_form .province-cell.active').closest('li').addClass('err_active');
          msgErrAnimate($('#billing_address_form .province-cell.active').find('.msg'));
          issubmit = false;
        }
...
function shippingSession(shippingId, shippingMoney, cart_res, isClick) {
  ...
  if (isClick) {
    ...
    $('#tax').text(setmeal.symbol + formatMoneyRate(total_price * taxVal)).attr('data-money', formatMoneyRate(total_price * taxVal))
            ...
            if (isClick) {
              if (cartRes.invalidGoodsList.length) {
                layui.config({
                  ...
                  var $ = layui.jquery,
                    layer = layui.layer
                  let tip = "Un Product supplier, Velez choose un product."
                  layer.confirm(tip, {
                    id: 'tip_code',
                    icon: 5,
                    title: '',
          ...
          layui.config({ version: true, base: '/layui', })
            .use(['jquery', 'layer'], function () {
              ...
              layer.open({
                closeBtn: 0,
                title: false,
                content: "De sole, c'est un marché.",
                btn: ['confirm'],
...
function country(countryId, countryKey) {
  ...
  $('#' + countryId).find('.address_country').html($html).addClass('is-change');
}

$(window).resize(function () {
  $('#order-info').css('bottom', '0')
});
