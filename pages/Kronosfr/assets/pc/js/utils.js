...
const itemizeSize = 8 // 每页显示个数
let itemizePage = 2 // 查看更多时的每次加载数据的倍数
let beforeLinkId = 1
...
function renderRecommendGoods(data, fnode) {
  return new Promise((resolve, reject) => {
    ...
    for (let i = 0; i < data.length; i++) {
      if (!$.isEmptyObject(data[i])) {
        html += `<div class="swiper-slide" sort="${data[i].sort}" start="${data[i].discountStaTime}" end="${data[i].discountEndTime}">
                  <a class="goods-info-wrap" href="productDetail_${data[i].id}.html">
                  <div class="img-box cover_wrap"><div class="quick-shop">Achetez maintenant</div>`
        if (data[i].freeShipping !== 0) {
          html += '<div class="discount-shipping">Livraison Gratuite</div>'
        }
        if (data[i].discount && data[i].discount != 10) {
          html += `<div class="discount-percent">
                    <div>${formateDiscount(100 - data[i].discount * 10)}%</div>
                    <div class="discount-off">RABAIS</div>
                  </div>`
        } else {
          if (data[i].originalPrice && data[i].originalPrice > data[i].defaultPrice) {
            html += `<div class="discount-percent">
                      <div>${formateDiscount(((data[i].originalPrice - data[i].defaultPrice) / data[i].originalPrice) * 100)}%</div>
                      <div class="discount-off">RABAIS</div>
                    </div>`
          }
        }
        if (data[i].otherImg) {
          html += `<img class="goods-img mainRelationListImg" src="" data-suffix-img="${data[i].mainImg}" alt="">`
          html += `<img class="associateRelationListImg goods-img" src="" data-suffix-img="${data[i].otherImg}" alt="">`
        } else {
          html += `<img class="goods-img" src="" data-suffix-img="${data[i].mainImg}" alt="">`
        }
        html += `</div>
                 <div class="goods-name">${data[i].goodsName}</div>`
        html += `<div class="price-info">`
        if (data[i].discount && data[i].discount != 10) {
          html += `<div class="original-price" p-price="${data[i].defaultPrice}"></div>
                   <div class="current-price" p-price="${(data[i].defaultPrice * data[i].discount) / 10}"></div>`
        } else {
          if (data[i].originalPrice && data[i].originalPrice > data[i].defaultPrice) {
            html += `<div class="original-price" p-price="${data[i].originalPrice}"></div>
                     <div class="current-price" p-price="${data[i].defaultPrice}"></div>`
          } else {
            html += `<div class="current-price" p-price="${data[i].defaultPrice}"></div>`
          }
        }
        html += '</div></a></div>'
      } else {
        html += `<div style="opacity:0"  class="swiper-slide" start="${data[i].discountStaTime}" end="${data[i].discountEndTime}"></div>`
      }
    }
    $(fnode)
      .html(html)
  ...
  return new Promise((resolve, reject) => {
    ...
    for (let i = 0; i < data.length; i++) {
      if (!$.isEmptyObject(data[i])) {
        ...
        var html1 = `<div   class="product-item  ${classNmae}" sort="${data[i].sort}" start="${data[i].discountStaTime}" end="${data[i].discountEndTime}">
      
        <a href='productDetail_${data[i].id}.html${promotionLogo}' class="loading">
          <div class="madal alginCenter">
            <button onclick="routerJump('productDetail_${data[i].id}.html${promotionLogo}')">Achetez maintenant</button>
          </div>`
        if (data[i].discount && data[i].discount != 10) {
          var html2 = `<div class="off discountLabel"><p>${formateDiscount(100 - data[i].discount * 10)
            }%</p><p>RABAIS</p></div>`
          ...
          if (data[i].originalPrice && data[i].originalPrice > data[i].defaultPrice) {
            var html2 = `<div class="off discountLabel ${data[i].freeShipping !== 0 ? 'freeShipping' : ''}"><p>${formateDiscount(((data[i].originalPrice - data[i].defaultPrice) / data[i].originalPrice) * 100)
              }%</p> <p>RABAIS</p></div>`
            // 需要显示折扣价格
        ...
        if (data[i].freeShipping !== 0) {
          var html3 = `<div class="freeLabel">Livraison Gratuite</div>`
        } else {
          var html3 = ''
        }
        if (data[i].otherImg) {
...
function render(data, type) {
  ...
  renderhtml(newData, '.productContainer', type).then(() => {
    changeMargin()
  })
}

function singleRender(data, type, size = itemizeSize, linkId) {
  const belongSideCategory = flatSideList(SIDEDATA).find(item => item.linkId == linkId)
  data = delItemsFn(data)
  data = [...prevArr, ...data]
  const newData = getpagination(
    paginationObj.currentPage,
  ...
  renderhtml(newData, `.productContainer_${linkId}`, type).then(() => {
    $(`.productContainer_${linkId}`)
      .prepend(
        `<div class="productContainer-name" id="anchor_${linkId}">
      <i></i>
      <span>${belongSideCategory.name}</span>
      <i></i>
    </div>`)
    const goodsList = findCategory(CATEGORYDATA.categoryList, Number(linkId)).goodsList
    // 重新渲染进来时，itemizePage值已经增加 所以减1
    if (goodsList.length > size && size < goodsList.length) {
      $(`.productContainer_${linkId}`).append(`<div onclick="loadMore(${linkId})" class="productContainer-more" moreid="${linkId}" length="${size}">View  More</div>`)
    } else {
      $(`.productContainer_${linkId}`).css({
        marginBottom: 0
      })
      $(`div[moreid=${linkId}]`).css({
        display: 'none'
      })
    }
    itemizePage++
...
// 渲染商品列表(新版渲染,不分页)
function newRender(data, linkId, type, name) {
  data = delItemsFn(data)
  data = [...prevArr, ...data]
  const newData = getpagination(
    1, itemizeSize,
  ...
  if (linkId != null) {
    $('.productContainer').append($(`<div class="productContainer_${linkId} productContainer-culumn" ></div>`))
    renderhtml(newData, `.productContainer_${linkId}`, type).then(() => {
      $(`.productContainer_${linkId}`)
        .prepend(
          `<div class="productContainer-name" id="anchor_${linkId}">
      ...
      if (goodsList.length > 8) {
        $(`.productContainer_${linkId}`).append(`<div onclick="loadMore(${linkId})" class="productContainer-more" name="${name}" moreid="${linkId}">View  More</div>`)
      } else {
        $(`.productContainer_${linkId}`).css({
          marginBottom: 0
        })
    ...
    $('.productContainer').append(
      `<div class="productContainer-empty">
          <div class="productContainer-name">
            <i></i>
            <span>${name}</span>
            <i></i>
          </div>
...
function renderSideList(id) {
  const mySideData = SIDEDATA.find(item => item.linkId == id)
  console.log(mySideData);
  if (mySideData && mySideData.childList && mySideData.childList.length > 0) {
    const sideLiItem = mySideData.childList.map(item => {
      return `<li linkId=${item.linkId}  linkType=${item.linkType} linkUrl=${item.linkUrl}>${item.name}</li>`
    })
    $('.productContainer-side').css('display', 'block').empty().html(
      `<ul>${sideLiItem.join('')}</ul>`
    )
  } else {
    $('.productContainer-side').css('display', 'none')
  }

}

function filterLevelData(arr, v) {
...
function renderRecommendEmpty() {
  ...
  renderRecommendGoods(newData, '#empty_swiper .swiper-wrapper').then(
    () => {
      if (!emptySwiper) {
...
function loadMore(id, name) {
  const activeSortEl = $('#sortBy li.active')
  // 重置翻页倍数
  if (beforeLinkId != id) {
    itemizePage = 2
    beforeLinkId = id
  }
  const data = findCategory(CATEGORYDATA.categoryList, Number(id))
  searchResult = data.goodsList.filter(item => exemptSearchList.indexOf(item.id) === -1)
  // callbackSort(data.sortRule, data.sortMode)
  searchResult = sort(searchResult, activeSortEl.attr('value'), activeSortEl.attr('sortMode'))
  singleRender(searchResult, 1, itemizeSize * itemizePage, id, name)
}